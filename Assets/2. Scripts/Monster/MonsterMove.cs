...
public class MonsterMove : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            currentLookDir = (Waypoints[0].position - transform.position).normalized;
        }
    ...
    void Update()
    {
        ...
        if (Waypoints != null && Waypoints.Length > 0)
        {
            ...
            TargetAnchor = Waypoints[currentWaypointIndex];
            float dist = Vector3.Distance(transform.position, TargetAnchor.position);
            if (dist < 0.1f)
            {
                ...
                {
                    currentWaypointIndex = 0; // 무한 반복 
                }
                Vector3 nextWaypointDir = (Waypoints[currentWaypointIndex].position - transform.position).normalized;
        ...
        {
            // 이동 방향은 타겟을 향해
            Vector3 moveDir = (TargetAnchor.position - transform.position).normalized;
