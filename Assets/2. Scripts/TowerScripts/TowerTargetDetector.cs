...
public class TowerTargetDetector : MonoBehaviour
{
    ...
    private List<MoveTest> testEnemies = new List<MoveTest>();
    ...
    public void RegisterEnemy(MoveTest enemy)
    {
    ...
    public MoveTest FindNearestEnemyInRange(
        Vector2Int towerTile,
        int range,
        Vector2Int baseCampTile)
    {
        MoveTest bestTarget = null;
        int minBaseDistance = int.MaxValue;

        //Debug.Log($"[감지기] 사거리 내 적 탐색 시작 | 전체 적 수: {enemies.Count}");

        foreach (MoveTest enemy in testEnemies)
        {
            if (enemy == null) continue;
